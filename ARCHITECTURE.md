...
**`controller/`**
- `StockController` — the only `@RestController`, base path `/api/stocks`. Wires 8 services into **14 endpoints**:
  ...
  - `GET /awards/progress` → `AwardEnrichmentProgressTracker.Snapshot`, polled by the frontend while `/awards` rows are still being enriched
  - `GET /results/upcoming` / `GET /results/announced` → `ScreenerResultsCalendarService`
  - `GET /results/announced/progress` → `ResultsEnrichmentProgressTracker.Snapshot`, same polling pattern as `/awards/progress`
...
- `AwardEnrichmentProgressTracker` / `ResultsEnrichmentProgressTracker` — two independent `@Component` singletons, each a generation-token-guarded counter (`generation`/`total`/`completed`) so the Awards and Results Calendar tabs can each poll `GET .../progress` for a live "N of M enriched" readout while their respective per-row fan-outs are in flight. Kept as separate instances (not shared) so one tab's in-flight request can't reset the other's counter mid-poll.
- `ScreenerResultsCalendarService` + `ResultExpectationCalculator` — **replaces `BseResultsCalendarService` (deleted).** No longer mines BSE's generic Board Meeting announcements feed by regex; instead reads the user's own authenticated Screener.in account's `/results/latest/` page directly via `ScreenerAuthService`, one day at a time (`result_update_date__day/month/year` query params — confirmed live to actually filter by day). Verified live that Screener has **no forward-looking results feed at all** (`/results/calendar/` doesn't exist), so `fetchUpcomingResults` always returns an empty page rather than fabricating data. Market-cap filtering (`MIN_MARKET_CAP_CR=200`) is done client-side in Java on the already-parsed per-row value, because adding Screener's own `mcap_min`/`mcap_max` params silently breaks the day filter (see Gotchas). `ResultExpectationCalculator` is unchanged and still an explicitly self-referential heuristic — it compares a company's trailing quarterly profit trend against itself, it is **not** analyst-consensus beat/miss, despite the "Beat Trend"/"Below Trend" naming. `AnnouncedResultResponse` also carries both `qoqProfitGrowthPercent` and `yoyProfitGrowthPercent` so the frontend can show quarter-over-quarter and year-over-year profit growth side by side. See `FUNCTIONALITY.md` for the full mechanics.
...
**`config/`** — `CorsConfig` (allowed origins are configurable via `app.cors.allowed-origins`, defaulting to the local dev ports — set it to your deployed frontend's origin before hosting), `WebClientConfig` (two near-identical browser-UA-spoofing `WebClient` beans for Yahoo/NSE), `CacheConfig` (6 named Caffeine caches — see table below), `OpenApiConfig` (Swagger UI at `/swagger-ui.html`). Standard Spring Boot wiring, nothing scoring-specific.
...
- **No prod frontend origin is baked in** — `app.cors.allowed-origins` defaults to the local dev ports; set it in your deployment environment before hosting this publicly (see README).
